...
df_new = pd.DataFrame(stock_list, columns=rs.fields)
print(f"Baostock 返回 {len(df_new)} 只证券")


# 只保留 A 股（代码以 sh.6、sh.9、sz.0、sz.3 开头）
def is_a_stock(code):
    if code.startswith('sh.6') or code.startswith('sh.9'):
        return True
...
if len(new_stocks) > 0:
    ...
    # 显示新股列表
    print("\n新股列表:")
    for _, row in to_insert.head(20).iterrows():
...
# 更新 daily 表中新股的名称（如果它们已经有数据但 name 是 NULL）
print("\n更新日线表中的新股名称...")
