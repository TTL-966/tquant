...
主源使用东方财富 API（curl_cffi / requests），备用源使用同花顺页面抓取（正则解析）。
"""
...
# 重试配置
MAX_RETRIES = 3
MAX_RETRIES_EM = 2  # 东方财富重试次数（少一些，失败快速降级）
...
class FundFlowFetcher:
    ...
    主源：东方财富 API（curl_cffi / requests，单位：元 → 转换为万元）
    备用源：同花顺页面抓取（返回单位：万元，正则解析）
    ...
    def _fetch_from_eastmoney(self, pure_code: str) -> Optional[Dict]:
        ...
        for attempt in range(1, MAX_RETRIES_EM + 1):
            try:
                ...
                result = {
                    ...
                    "small_net": to_wan(latest[2]),
                    "medium_net": to_wan(latest[3]),
                    "big_net": to_wan(latest[4]),
                    "super_net": to_wan(latest[5]),
                    "source": "eastmoney",
                }
    ...
    def _fetch_from_tonghuashun(self, pure_code: str) -> Optional[Dict]:
        ...
        使用正则解析 HTML 表格（不依赖 class 名，更健壮），
        提取最新一条交易日的主力/超大单/大单/中单净流入（万元）。
        """
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                # 正则提取所有数据行（不依赖表格 class 名）
                trs = re.findall(r"<tr[^>]*>(.*?)</tr>", resp.text, re.DOTALL)
                data_rows = []
                for tr in trs:
                    tds = re.findall(r"<td[^>]*>\s*(.*?)\s*</td>", tr, re.DOTALL)
                    if not tds:
                        continue
                    cleaned = [re.sub(r"<[^>]+>", "", td).strip() for td in tds]
                    if re.match(r"^\d{8}$", cleaned[0]):
                ...
                if len(latest) < 13:
                    raise Exception(f"字段不足（共 {len(latest)} 列）")
    ...
    def _cache_get(self, pure_code):
        ...
        with self._cache_lock:
            ...
            if entry is None:
                return None
            ts_cache, data = entry
            if time.time() - ts_cache < self._cache_ttl:
                return data
