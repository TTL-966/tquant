        ...
        for i in range(20, len(df)):
            if (df['ma5'].iloc[i-1] <= df['ma20'].iloc[i-1] and df['ma5'].iloc[i] > df['ma20'].iloc[i]):
                ...
                if shares > 0:
                    ...
                    if cost <= cash:
                        signals.append({"date": df['trade_date'].iloc[i].strftime('%Y-%m-%d'), "code": code, "type": "buy", "price": round(price, 2), "shares": shares})
                        cash -= cost
                        holdings += shares
            elif (df['ma5'].iloc[i-1] >= df['ma20'].iloc[i-1] and df['ma5'].iloc[i] < df['ma20'].iloc[i] and holdings > 0):
                price = df['close'].iloc[i]
                shares = min(holdings, shares_per_trade)
                if shares > 0:
                    signals.append({"date": df['trade_date'].iloc[i].strftime('%Y-%m-%d'), "code": code, "type": "sell", "price": round(price, 2), "shares": shares})
